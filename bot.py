...
async def add_role_member(message: discord.message.Message, log):
    ...
    log.write(f"[{datetime.now()}] : [{user.name}] \t{message.content} is a PSG Student\n")

    
    if message.content.upper() in userRoleMapping.keys():

        nickname = f'{userRoleMapping[message.content.upper()]["name"]} | {userRoleMapping[message.content.upper()]["event"]}' 
        ...
        await user.send("Your request cannot be processed. Please contact your Event Coordinator or Admin to gain access")
        log.write(f"[{datetime.now()}] : [{user.name}] \t{message.content} Not available in database\n" )
...
async def add_role_participant(message: discord.message.Message, log):
    ...
    if events:
        ...
        log.write(f"[{datetime.now()}] : [{user.name}] \t`{message.content}` is a Not a Registered Participant\n")
        await user.send("Oh oohh!, it seems like you haven't registered for Login 2022. Please Register through our website and try again later! If this continues, please contact Server Admin\n\n Website : https://psglogin.in") 
...
async def on_message(message: discord.message.Message):
    ...
    if message.channel.id == int(os.getenv("CHANNEL_ID")):
        ...
        log.write(f"[{datetime.now()}] : [{bot.get_user(message.author.id).name}] \tMessage Deleted `{message.content}`\n")
